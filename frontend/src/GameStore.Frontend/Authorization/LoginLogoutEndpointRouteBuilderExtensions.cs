...
internal static class LoginLogoutEndpointRouteBuilderExtensions
{
    ...
    )
    {
        ...
        group
            .MapGet(
                "/login",
                (string? returnUrl, IConfiguration configuration) =>
                {
                    var idp =
        ...
        group.MapPost(
            "/logout",
            ([FromForm] string? returnUrl, IConfiguration configuration) =>
            {
                var idp =
                    configuration["IdentityProvider"]
                    ?? throw new InvalidOperationException("Identity Provider not found");
                return TypedResults.SignOut(
                    ...
                    [CookieAuthenticationDefaults.AuthenticationScheme, idp]
                );
            }
        );
